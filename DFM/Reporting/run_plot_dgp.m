...
for nf=1:length(lags_folders) % For each folder...
    ...
    for ne=1:length(exper_files) % For each experiment in folder...
        ...
        if i_exper_combine == 1
            fig_irfs = figure('Position', [100 100 500*n_exper_combine 400]);
        end
        figure(fig_irfs);
        subplot(1,n_exper_combine,i_exper_combine);
        ...
        hold off
        title(exper_plotname,'interpreter','latex','fontsize',8/7*font_size)
        xlabel('Horizon','interpreter','latex','FontSize',8/7*font_size)
