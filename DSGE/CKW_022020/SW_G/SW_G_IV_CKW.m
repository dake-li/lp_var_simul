...
settings.est.shock_optim         = 0; % choose optimal linear combination of shocks?
...
SW_model.persistency = compute_persist_DSGE(SW_model,settings);
...
save('SW_G_IV_CKW','SW_model','settings','results','-v7.3');
...
for i_method = 1:settings.est.n_methods
    ...
    figure(plot_indx)
    plot(settings.est.IRF_select-1, SW_model.target_irf(:,settings.specifications.plot_indx),'Linewidth',5)
    hold on
