...
settings.specifications.manual_var_select = [12 1]; % manually select specifications
...
settings.specifications.random_n_var = 2; % number of variables in each random specification
...
settings.est.shock_optim_var     = 12; % for which variable in full model to choose optimal linear combination of shocks 
...
settings.est.IRF_select           = 1:21; % which IRFs to summarize
...
settings.est.methods_name = {'svar','svar_corrbias','bvar','lp','var_avg','svar_iv','lp_iv'}; % choose estimands
...
settings.est.irfLimitOrder = 2; % shrink towards polynomial of that orders
...
settings.est.full_methods_name = {'svar','svar_corrbias','bvar','lp','var_avg','svar_iv','lp_iv'};
...
clear temp_lambda_lp_penalize temp_weight_var_avg temp_F_stat_svar_iv temp_F_stat_lp_iv
...
clear results_lambda_lp_penalize results_weight_var_avg results_F_stat_svar_iv results_F_stat_lp_iv
clear i_method thisMethod
...
save('SW_DFM_G_IV_CKW','DF_model','settings','results','-v7.3');
toc;
